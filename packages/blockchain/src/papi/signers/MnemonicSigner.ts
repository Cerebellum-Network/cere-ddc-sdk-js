 ...
 * An empty/blank phrase is rejected rather than silently falling back to a
 * well-known public dev phrase, to avoid a silent wrong-key footgun.
 *
...
export class MnemonicSigner implements CereSigner {
  ...
  constructor(uri: string, opts: { type?: 'sr25519' | 'ed25519' } = {}) {
    ...
    if (!trimmedPhrase) {
      throw new Error('MnemonicSigner: empty mnemonic/URI');
    }

    const mini = entropyToMiniSecret(mnemonicToEntropy(trimmedPhrase));

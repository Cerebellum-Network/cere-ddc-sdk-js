...
An object that includes the access token, the CID of the file to retrieve, and an optional range.
...
> **putMultipartPiece**(`request`): `Promise`\<`Uint8Array`\<`ArrayBuffer`\>\>
...
An object that includes the access token, the part size, and the piece to store.
...
> **putRawPiece**(`metadata`, `content`): `Promise`\<`Uint8Array`\<`ArrayBuffer`\>\>
...
An object that includes the access token and the metadata for the raw piece.

##### content

`Content`

The content of the raw piece as a `Content` object.
